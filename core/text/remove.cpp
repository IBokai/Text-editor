...
void Text::remove(size_t start_pos, size_t end_pos){
    ...
    memcpy(temp_left, text, start_pos);
    memcpy(temp_right, text + end_pos + 1, size);
    delete[] text;
    text = new char[size - (end_pos - start_pos + 1) + 1];
    memcpy(text, temp_left, start_pos);
    delete[] temp_left;
    memcpy(text + start_pos, temp_right, size-end_pos - 1);
    delete[] temp_right;
    text[size - (end_pos - start_pos + 1)] = '\0';
    size = size - end_pos + start_pos;
}